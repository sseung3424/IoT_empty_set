...
def set_input_tensor(interpreter, image_rgb):
    ...
    interpreter.set_tensor(d["index"], np.expand_dims(img, 0))

def get_output(interpreter, conf_thresh=0.3):
    out = interpreter.get_output_details()
    boxes   = interpreter.get_tensor(out[0]["index"])[0]
    classes = interpreter.get_tensor(out[1]["index"])[0].astype(np.int32)
    ...
    for i in range(count):
        ...
        res.append({"bbox": (float(xmin), float(ymin), float(xmax), float(ymax)),
                    "score": float(scores[i]), "class_id": int(classes[i])})
    return res
...
class Follower:
    ...
    def stop(self): self.car.Car_Stop()

    def drive_wheels(self, x_dev, y_max):
        if y_max > STOP_NEAR_Y:
            self.stop()
            return "Stop"
        delta = 0 if abs(x_dev) < CENTER_DEADZONE else int(Kx * x_dev)
...
def run_tracking(stop_event):
    ...
    try:
        while not stop_event.is_set():
            ...
            interpreter.invoke()
            dets = get_output(interpreter, CONF_THRESH)
            best = max([d for d in dets if labels.get(d["class_id"]) == TARGET_LABEL],
                       key=lambda x: x["score"], default=None)

            status = "Idle"
            pan_cmd, tilt_cmd = follower.pan, follower.tilt

            if best is not None:
                follower.state = "TRACKING"
                follower.last_seen_ts = time.monotonic()
                xmin, ymin, xmax, ymax = best["bbox"]
                x_center = (xmin + xmax) * 0.5
                y_center = (ymin + ymax) * 0.5
                x_dev = 0.5 - x_center
                status = follower.drive_wheels(x_dev, y_max=ymax)
                pan_cmd, tilt_cmd = follower.aim_servos(x_center, y_center)
            else:
                if follower.state == "TRACKING" and time.monotonic() - follower.last_seen_ts > LOST_TIMEOUT:
                    print("[tracking] target lost → SEARCH")
                    follower.state = "SEARCHING"
                    follower.stop()
                if follower.state == "SEARCHING":
                    status = "Searching..."
                    pan_cmd, tilt_cmd = follower.aim_servos(0.5, 0.5)
            ...
            cv2.putText(bgr, f"STATUS: {status}", (10, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,255), 2)
            cv2.putText(bgr, f"PAN={int(pan_cmd)} TILT={int(tilt_cmd)}", (w-210, 22),
