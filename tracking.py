...
# 2m 근처 유지: 더 먼 거리 ⇒ 박스 높이 목표를 더 작게
INIT_TARGET_FOLLOW_H = 0.22
NEAR_ENGAGE_FACTOR   = 1.18
NEAR_RELEASE_FACTOR  = 0.95
...
TUNE_WINDOW_S      = 2.5
TUNE_MIN_SAMPLES   = 20
TUNE_MIN_H, TUNE_MAX_H = 0.08, 0.85
...
# --- 검색 패턴(Searching) ---
SEARCH_SPIN_SPEED   = 45     # 좌/우 회전 속도
SPIN_RATE_DEG_PER_SEC_AT_100 = 6.0
SEARCH_FULL_TURN_MARGIN = 1.10

# Step-and-Stare (회전-정지 주기)
SEARCH_BURST_MS = 4000   # 회전 유지 시간: 4초
SEARCH_HOLD_MS  = 140    # 정지 유지 시간

# (서보 팬 스윕 비사용; Searching에선 차체 회전만 사용)
# ----------------------------------------------------
...
class Follower:
    ...
    def _maybe_finish_tune(self):
        ...
        if (now - self.tune_start_ts) >= (TUNE_WARMUP_S + TUNE_WINDOW_S) and len(self.tune_buf) >= TUNE_MIN_SAMPLES:
            ...
            self.near_engage  = self._clamp(self.target_follow_h * NEAR_ENGAGE_FACTOR, 0.10, 0.90)
            self.near_release = self._clamp(self.target_follow_h * NEAR_RELEASE_FACTOR, 0.05, self.near_engage - 0.02)
            self.tune_active = False
    ...
    # -------- 검색 패턴: 차체 회전만 사용 (4초 회전 / 140ms 정지) --------
    def searching_step(self):
        now = time.monotonic()

        # 서보는 중앙으로 유지
        self.aim_servos_to_angles(PAN_CENTER, TILT_CENTER)
            ...
            if best is not None:
                ...
                if dt_since_seen <= LOST_TIMEOUT:
                    ...
                    if follow.state != "SEARCHING":
                        ...
                        follow.stop()
                        # 서보 중앙으로 정렬
                        follow.aim_servos_to_angles(PAN_CENTER, TILT_CENTER)
