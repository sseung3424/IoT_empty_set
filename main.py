...
MANUAL_INPUT     = int(os.environ.get("MANUAL_INPUT", "1"))   # 1: 키보드 입력, 0: 마이크(STT)
...
from tracking import run_tracking  # 아래 새 run_tracking(stop_event)로 교체됨
...
def main():
    ...
    conv_thread.start()
    log("conv thread started")

    global threads
    threads = [track_thread, conv_thread]

    # 메인스레드는 종료 제어만
    try:
        while not stop_event.is_set():
            # 둘 중 하나라도 비정상 종료되면 전체 종료
